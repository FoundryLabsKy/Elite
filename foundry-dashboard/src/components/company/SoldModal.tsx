...
export function SoldModal({ company, onClose }: SoldModalProps) {
  ...
  const [domainChoice, setDomainChoice] = useState(OTHER);
  ...
  useEffect(() => {
    if (company) {
      setSalePrice(company.sale_price != null ? String(company.sale_price) : "");
      setMonthlyFee(company.monthly_fee != null ? String(company.monthly_fee) : "30");
      const existing = company.final_domain;
      if (existing && company.potential_domains.includes(existing)) {
        setDomainChoice(existing);
        setCustomDomain("");
      } else {
        setDomainChoice(company.potential_domains[0] ?? OTHER);
        setCustomDomain(existing ?? "");
      }
      setFinalUrl(company.final_url ?? "");
    }
  }, [company]);
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!company || !valid || saving) return;
    setSaving(true);
